...
{
    public class BaseClockInController : Controller
    {
        private readonly IMenuMasterService _menuService;
        private readonly IUserMenuMappingService _userMenuMappingService;

        public BaseClockInController(IMenuMasterService menuService, IUserMenuMappingService userMenuMappingService)
        {
        ...
        {
            var designation = UserUtility.GetDesignation(HttpContext);
            if (designation == "Admin")
            {
                var menulist = await _menuService.GetAllMenuMasters();
                var btclockMenuList = menulist.Where(x => x.ModuleMasterId == 1).ToList();
                var topLevelItems = btclockMenuList.Where(m => m.ParentId == 0 || m.ParentId == null).ToList();
                foreach (var item in topLevelItems)
                {
                    item.Children = btclockMenuList.Where(m => m.ParentId == item.Menuid).ToList();
                }
            ...
            else
            {
                ...
                var userMenuList = userMenus.Where(x => x.MenuItem?.MenuMaster?.ModuleMasterId == 1).Select(x => x.MenuItemId);
                var menulist = await _menuService.GetAllMenuMasters();
